...
function renderDashboard() {
  ...
  renderKv($('runtimeKv'), [
    ...
    ['自动断点', runtime.autoGenerateCacheBreakpoints ? '开启' : '关闭'],
...
function renderCache() {
  ...
  $('autoGenerateCacheBreakpointsSwitch').checked = Boolean(runtime.autoGenerateCacheBreakpoints);
...
function renderAdvanced() {
  ...
  $('cacheControl').textContent = JSON.stringify({
    ...
    autoGenerateCacheBreakpoints: runtime.autoGenerateCacheBreakpoints,
...
function renderDetail() {
  ...
  renderMetaGrid($('detailMetaGrid'), [
    ...
    ['自动断点', item.gateway?.autoGenerateCacheBreakpoints
      ? `开启 · 新增 ${item.gateway?.cachePolicy?.autoGeneratedBreakpoints?.added ?? 0}`
      : '关闭'],
...
function bindEvents() {
  ...
  $('autoGenerateCacheBreakpointsSwitch').onchange = async () => { await postJson('/console/auto-cache-breakpoints', { enabled: $('autoGenerateCacheBreakpointsSwitch').checked }); await refreshAll(); setStatus($('autoGenerateCacheBreakpointsSwitch').checked ? '自动缓存断点已开启，锚点与 Prefix Lock 将重新学习' : '自动缓存断点已关闭，锚点与 Prefix Lock 将重新学习'); };
