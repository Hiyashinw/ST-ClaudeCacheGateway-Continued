...
- **数字模式**：填写非负整数 `x`。如果从头到尾共有 20 个候选锚点，`x=4` 时只把前 16 个候选交给后续选择策略，最新 4 个不参与网关选点；`x=0` 表示不裁剪。若 `x` 大于候选总数，全部网关候选都会被忽略。调用方原有的 `cache_control` 不会被删除。
