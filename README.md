...
- 支持高级配置：全局 Usage 文字/背景配色，以及按渠道保存的包含 / 排除主体参数和请求头。
...
策略设置会持久化到 `gateway-settings.json`。启动时先读取 `default-gateway-settings.json` 的全局缺省值，再由 `gateway-settings.json` 中存在的全局字段覆盖；默认文件中的渠道字段会被忽略，渠道仍只来自用户文件或内置模板。旧配置会在运行时迁移到 `schemaVersion: 10`：schema 8 的系统身份消息处理值 `default / anthropic` 都迁移为新的 `default`，`top` 迁移为 `off`；更早版本的 `moveSystemMessagesToTop` 布尔值 `false / true` 分别迁移为 `default / off`，缺少旧字段时使用 `default`。固定头、锚点和滚动间隔缺省为 `0 / off / 3`；旧的自动断点布尔值 `true / false` 分别迁移为 `on / off`，缺省仍为 `off`；旧的空值、`default`、`provider-default` 和 `none` TTL 会迁移为 `auto`；新增的末锚点 5m 开关默认关闭，忽略末尾锚点默认数字模式 x=0；Usage 外观缺失或字段不完整时会补齐默认配色，且始终作为全局设置而非渠道字段。0–5 是 x 的建议范围，经用户明确确认的更大结果也会持久化并在重启后恢复。已学习的锚点、轮换进度、上下文索引、块哈希历史、评估样本和诊断正文只保存在内存里；评估途中重启会从 x=0 和第 1 次请求重新开始，锚点也会重新学习，诊断开关则保持上次设置。不需要新增环境变量。
...
控制台可分别设置输入、输出、缓存命中、缓存创建，以及四档缓存命中率的文字色和背景色。背景默认透明；修改时会即时预览，点击“应用配色”后写入 `gateway-settings.json`，切换渠道不会改变配色。“恢复默认”会恢复内置的绿 / 紫 / 蓝 / 黄与命中率分档颜色。
