...
- **支持更新的Anthropic模型，同时对老模型有特殊优化**：原项目针对Anthropic 上游的兼容方式非常简单粗暴，即将system身份消息统一上移合并。该项目针对更新的Anthropic模型（如Fable-5.1，原生支持跨轮系统消息，即可以在用户、AI助手中间插入系统身份的消息）时，对提示词不加以额外处理。而对应老模型（如Opus-4.6），则默认将system身份消息转换为user身份消息，来保证提示词至少在上下文正确位置。
