...
- **固定头缓存点**：可优先保留最前面的 0–4 个合格候选断点，避免候选持续增长时稳定头部断点被上游四点上限挤掉；若“忽略末尾锚点”的裁剪范围覆盖到固定头，则以该字面裁剪结果为准。
...
“末锚点自动使用 5 分钟”会在最终选中的网关锚点中找到最末的 1h 控制，并仅将它改为 `ttl: "5m"`；已有调用方控制点不会被覆盖，若修改会违反 1h 在前、5m 在后的顺序则跳过并记录原因。

“忽略末尾锚点”设置为数字时，若共有 20 个候选锚点且 x=4，网关只把前 16 个候选交给选择策略，最新 4 个不参与网关选点；x 大于候选总数时，全部网关候选都会被忽略，调用方原有的 `cache_control` 不会被删除。评估模式会先把实际 x 设为 0，并提示连续完成 3 次同渠道、同协议、同模型的成功对话；范围变化会重新计数，失败请求不计入。第三次成功后，网关比较三份有序块哈希，找到最早发生变化、新增或缺失的块，并把该块自身及其后的候选锚点数作为结果。结果 0–5 会自动填入并持久化；结果大于等于 6 会保留真实值并提示检查预设，只有用户明确点击“仍填入 x=N”后才会保存，重新评估则再次从 x=0 开始。缺少可比较哈希时不会自动填入。

启用任一新策略后，断点按以下优先级选择：

1. 尾部裁剪后仍合格的最前面固定头候选点。
2. 已冻结的活动锚点，以及本次单槽替换产生的新锚点。
3. 关闭/单锚点模式的最新普通尾点，或滚动模式首次学习时用于填满剩余预算的尾点。
...
策略设置会持久化到 `gateway-settings.json`。启动时先读取 `default-gateway-settings.json` 的全局缺省值，再由 `gateway-settings.json` 中存在的全局字段覆盖；默认文件中的渠道字段会被忽略，渠道仍只来自用户文件或内置模板。旧配置会在运行时迁移到 `schemaVersion: 9`：schema 8 的系统身份消息处理值 `default / anthropic` 都迁移为新的 `default`，`top` 迁移为 `off`；更早版本的 `moveSystemMessagesToTop` 布尔值 `false / true` 分别迁移为 `default / off`，缺少旧字段时使用 `default`。固定头、锚点和滚动间隔缺省为 `0 / off / 3`；旧的自动断点布尔值 `true / false` 分别迁移为 `on / off`，缺省仍为 `off`；旧的空值、`default`、`provider-default` 和 `none` TTL 会迁移为 `auto`；新增的末锚点 5m 开关默认关闭，忽略末尾锚点默认数字模式 x=0。0–5 是 x 的建议范围，经用户明确确认的更大结果也会持久化并在重启后恢复。已学习的锚点、轮换进度、上下文索引、块哈希历史、评估样本和诊断正文只保存在内存里；评估途中重启会从 x=0 和第 1 次请求重新开始，锚点也会重新学习，诊断开关则保持上次设置。不需要新增环境变量。
...
- Usage 的输入 token、输出 token、缓存创建、缓存命中和命中率；Anthropic 会把未缓存、读取和创建输入按协议口径合并计算。

- 最终发给上游的请求体
- 请求头摘要
- 缓存断点位置
- 最终入选的每个断点及其选择原因（固定头、冻结活动锚点、单槽替换新锚点、最新尾点或调用方已有控制）
